...
from src.secrets import BitwardenSecretManager
...
def initialize_components():
    ...
    secrets = BitwardenSecretManager(
    ...
    slack_app = App(
        token=secret_values.get('SLACK_BOT_TOKEN') or os.getenv('SLACK_BOT_TOKEN'),
        signing_secret=secret_values.get('SLACK_SIGNING_SECRET') or os.getenv('SLACK_SIGNING_SECRET')
    )
    
    ai_assistant = AIAssistant(
        api_key=secret_values.get('GOOGLE_GEMINI_API_KEY') or os.getenv('GOOGLE_GEMINI_API_KEY')
    )
    
    trello_workflow = TrelloWorkflow(
        api_key=secret_values.get('TRELLO_API_KEY') or os.getenv('TRELLO_API_KEY'),
        api_secret=secret_values.get('TRELLO_API_SECRET') or os.getenv('TRELLO_API_SECRET'),
        token=secret_values.get('TRELLO_TOKEN') or os.getenv('TRELLO_TOKEN')
    )
    
    logger.info("All components initialized successfully")
    return slack_app, ai_assistant, trello_workflow

def register_event_handlers(app, ai_assistant, trello_workflow):
    ...
    def handle_team_join(event, say):
        ...
        admin_channel = "C0123456789"  # Replace with actual channel ID
...
def main():
    ...
    try:
        ...
        slack_app, ai_assistant, trello_workflow = initialize_components()
        
        # Register event handlers
        register_event_handlers(slack_app, ai_assistant, trello_workflow)
